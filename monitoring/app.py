...
def load_reference_dataset(ref_filename: str) -> pd.DataFrame:
    try:
        df = pd.read_csv(ref_filename)
        # new_df = df[cols_to_keep].copy()
        # new_df = new_df.rename(columns=col_rename)
        df = df.reindex(columns=cat_features + num_features + target)
        df["attrition_flag"] = df["attrition_flag"].apply(lambda x: 1 if x == "Attrited Customer" else 0)
        df.rename(columns={"attrition_flag": "prediction"}, inplace=True)
...
class MonitoringService:
    ...
    def iterate(self, dataset_name: str, new_rows: pd.DataFrame):
        ...
        # logging.info(f'Inside iterate {self.column_mapping[dataset_name]}')
...
def configure_service():
    ...
    for dataset_name, dataset_options in config["datasets"].items():
        ...
        reference_data = load_reference_dataset(reference_file)
